...
typedef enum
{
    ...
    UART_FLAG_OR = (unsigned char)0x08,   /* Overrune / LIN
                                             slave error */
...
typedef enum
{
    UART_DATA_SIZE_8_BITS = 0x00U,
    UART_DATA_SIZE_9_BITS = 0x10U
} uart_data_size_t;

typedef enum
{
    UART_PARITY_DISABLE = 0x00U,
    UART_PARITY_ENABLE = 0x02U
} uart_parity_t;

typedef enum
{
    UART_PARITY_SELECTION_ODD = 0x01U,
    UART_PARITY_SELECTION_EVEN = 0x00U
} uart_selection_t;
...
error_t uart_init(uart_baudrate_t baudrate,
                  uart_data_size_t word_length, uart_parity_t parity,
                  uart_selection_t selection, uart_stopbit_t stopbit);
